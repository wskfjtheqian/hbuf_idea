...
public class EnumFieldIdCollateFix extends BaseIntentionAction {
    private final HbufIdElement element;

    public EnumFieldIdCollateFix(HbufIdElement element) {
    ...
    public void invoke(@NotNull final Project project, final Editor editor, PsiFile file) throws IncorrectOperationException {
        WriteCommandAction.writeCommandAction(project).run(() -> {
            ArrayList<HbufEnumFieldElement> elements = new ArrayList<>(HbufUtil.getEnumByChild(element).getEnumBody().getEnumFieldList().getFields());
